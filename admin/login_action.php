...
if(isset($_POST['submit'])){  
	...
	$md5pass = md5($pass);
	 $sql = "SELECT * FROM `xxx_user` WHERE `user_name` = '$user' AND `PASS` = '$md5pass' AND `is_active` = 1 AND `active_from` <= '$currentTime' AND `active_to` >= '$currentTime'";
	...
	if(!empty($data)){
		...
		header('Location: soft/animal.php');
		exit;
	}else{
