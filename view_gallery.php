...
<html lang="en">
  <head>
  <?php
	if(isset($_GET['ear_tag:'])){
	$ear_tag = $_GET['ear_tag:'];
?>
  <?php 
	$sql = (mysqli_query($con,"SELECT * FROM animal_type WHERE ear_tag = '$ear_tag' "));
				while($row = mysqli_fetch_assoc($sql)){
					$ear_tag = $row['ear_tag'];
						   				
		?>
<title><?php echo $row['ear_tag']; ?></title>
				<?php } ?>
                            ...
                            <?php 
                              ...
                              <div class="nav-item" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> 
                                <?php 
                                  ...
                                  echo '<a class="nav-link waves-effect waves-dark" href="#"><img style="height:45px;width:45px; margin-right: -60px; margin-bottom: -50px;"" class="profile-pic img-circle" src="images/user.jpg"></a>'; ?><a class="nav-link" style="color:green; padding-left:60px; margin-top: -30px;"><?php echo $buyer_name ;?></a>
                                  <?php 

                              }
                               else{ echo 
                               '<a class="nav-link waves-effect waves-dark" href="#"><img style="height:45px; width:45px; margin-right: -60px; margin-bottom: -50px;" class="profile-pic img-circle" src="images/'?><?php echo $buyer_image;?>"> <a class="nav-link" style="color:green; padding-left:60px; margin-top: -30px;"><?php echo $buyer_name ;?></a><?php '</a>'; }}?>
        ...
        <div class="row">
          <div class="col-lg-8 ftco-animate">
					...
					<?php 
						...
						$sql = (mysqli_query($con,"SELECT * FROM animal_type LEFT JOIN add_animal ON add_animal.animal_no= animal_type.animal_no WHERE ear_tag='$ear_tag' "));
						while($row = mysqli_fetch_assoc($sql)){
							$ear_tag = $row['ear_tag'];
						   ...
						   $animal_type_no = $row['animal_type_no'];
						
					?>
          	<p>
              <img src="admin/soft/upload/<?php echo $row['image_animal']; ?>" alt="" class="img-fluid">
            </p>
			
            <h1 class="mb-3"><?php echo $row['animal_name']; ?></h1>
			<h2>Ear Tag:</h2> <p><?php echo $row['ear_tag']; ?></p>
            <h2>Date of Birth:</h2><p><?php echo $row['dob']; ?></p>
            <h2>Color:</h2><p><?php echo $row['color']; ?></p>
            <h2>Breed:</h2><p><?php echo $row['breed']; ?></p>
            <h2>Weight:</h2><p><?php echo $row['weight']; ?></p>
